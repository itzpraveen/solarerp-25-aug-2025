...
async function main() {
  ...
  const TENANT_ID = process.env.SEED_TENANT_ID || (await (async () => {
    const { data, error } = await supabase.from('tenants').insert({ name: 'Demo Tenant' }).select('id').single();
    if (error || !data?.id) throw new Error(`Failed to create tenant: ${error?.message || 'unknown error'}`);
  ...
  const { data: cust, error: custErr } = await supabase
    .from('customers')
    .insert({ tenant_id: TENANT_ID, name: 'Harilal', phone: '+91-9xxxxxxxxx', address: 'Mampad PO' })
    .select('id')
    .single();
  if (custErr || !cust?.id) throw new Error(`Failed to insert customer: ${custErr?.message || 'unknown error'}`);

  // Sample job
  const { data: job, error: jobErr } = await supabase
    .from('jobs')
    .insert({ tenant_id: TENANT_ID, customer_id: cust.id, system_type: 'On-grid', status: 'Lead', capacity_kw: 5, location: 'Mampad' })
    .select('id')
    .single();
  if (jobErr || !job?.id) throw new Error(`Failed to insert job: ${jobErr?.message || 'unknown error'}`);

  // Sample proposal (with storage-like key)
  const proposalKey = `${TENANT_ID}/Q${Date.now()}_DEMO.pdf`;
  const { error: propErr } = await supabase.from('proposals').insert({ tenant_id: TENANT_ID, job_id: job.id, date: new Date().toISOString().slice(0,10), kit_name: 'On-grid 5 kW', price_before_tax: 265000, tax: 0, total: 265000, pdf_url: proposalKey });
  ...
  if (fs.existsSync(mockPath)) {
    ...
    const { error: upErr } = await supabase.storage.from('documents').upload(proposalKey, body, { contentType: 'application/pdf', upsert: true });
  ...
  const { error: invErr } = await supabase
    .from('invoices')
    .insert({ tenant_id: TENANT_ID, job_id: job.id, invoice_type: 'Deposit', date: new Date().toISOString().slice(0,10), amount_before_tax: 53000, tax: 0, total: 53000, status: 'Draft', due_date: new Date().toISOString().slice(0,10) });
