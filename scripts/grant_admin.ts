...
async function main() {
  ...
  if (!email) throw new Error('Usage: npm run grant:admin -- --email=user@example.com [--role=owner|admin] [--tenant=<uuid>]');
  ...
  if (!userId) {
    const { data: created } = await (admin as any).auth.admin.createUser({ email, email_confirm: true });
    userId = created?.user?.id || null;
  }
  if (!userId) throw new Error('Unable to resolve or create user for email: ' + email);

  // Resolve tenant
  let tenantId = tenant || process.env.TENANT || process.env.SEED_TENANT_ID || '';
  if (!tenantId) {
    const { data: ts } = await admin.from('tenants').select('id, name');
    const rows = (ts as any[]) || [];
    if (rows.length === 1) tenantId = rows[0].id as string;
    else {
      console.log('Tenants in this project:\n' + rows.map((r) => `${r.id}  ${r.name}`).join('\n'));
  ...
  const { error } = await admin
    .from('profiles')
    .upsert({ user_id: userId, tenant_id: tenantId, role: targetRole, display_name: email.split('@')[0] })
    .eq('user_id', userId);
  if (error) throw error;
  console.log(`Granted ${targetRole} to ${email} in tenant ${tenantId} (user ${userId}).`);
}
