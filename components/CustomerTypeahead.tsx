  ...
  return (
    <div className="relative" ref={boxRef}>
      <Input
        ...
        onChange={(e) => { setTerm(e.target.value); setOpen(true); setSel(0); }}
        onKeyDown={(e) => {
          if (!open) return;
          if (e.key === 'ArrowDown') { e.preventDefault(); setSel((v) => Math.min(v + 1, Math.max(0, rows.length - 1))); }
          if (e.key === 'ArrowUp') { e.preventDefault(); setSel((v) => Math.max(v - 1, 0)); }
          if (e.key === 'Enter') {
            e.preventDefault();
            if (rows[sel]) { onSelect(rows[sel]); setOpen(false); }
            else if (showCreate) { onCreateRequested(term.trim()); }
          }
      ...
      {open && (
        <div className="absolute z-30 mt-1 w-full rounded border bg-white text-sm shadow-sm dark:border-gray-800 dark:bg-gray-900">
          {loading && <div className="px-3 py-2 text-gray-600 dark:text-gray-300">Searching…</div>}
          {!loading && rows.map((r, i) => (
            ...
            >
              ...
              <span className="ml-2 text-xs text-gray-500">{r.phone || ''}</span>
            </button>
          ))}
          {!loading && showCreate && (
            <button type="button" className="w-full px-3 py-2 text-left text-blue-600" onClick={() => onCreateRequested(term.trim())}>
