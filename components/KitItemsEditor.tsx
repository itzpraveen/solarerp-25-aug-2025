...
export default function KitItemsEditor({ kitName }: { kitName: string }) {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      // Load available items in tenant for selection
      ...
      const [{ data: it }, { data: ks }] = await Promise.all([
        tenantId
          ? supabase.from('items').select('item_code, name, unit').eq('tenant_id', tenantId).order('name')
      ...
      setItems(((it as any[]) || []).map((x) => ({ item_code: x.item_code, name: x.name, unit: x.unit })));
  ...
  const add = async () => {
    ...
    setSaving(true);
    try {
      await supabase.from('kit_items').upsert({ kit_name: kitName, item_code: addCode, qty: Number(addQty) || 1 });
      setAddCode(''); setAddQty(1);
      await load();
    } catch (e: any) {
      setError(String(e?.message || e));
    } finally {
  ...
  return (
    <Card title="Kit Items">
      ...
      <div className="grid grid-cols-1 gap-2 md:grid-cols-4">
        ...
        <Input type="number" min={0} step={0.01} placeholder="Qty" value={addQty} onChange={(e) => setAddQty(Number(e.target.value))} />
              ...
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.item_code} className="border-b">
                    ...
                    <td className="p-2">
                      <Button variant="outline" size="sm" onClick={() => remove(r.item_code)}>Remove</Button>
                    </td>
                  </tr>
                );
              })}
