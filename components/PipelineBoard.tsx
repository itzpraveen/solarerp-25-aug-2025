...
export default function PipelineBoard({ branchId }: { branchId?: string | null } = {}) {
  ...
  const onDrop = async (jobId: string, newStatus: JobStatus) => {
    const ok = await confirm({ title: 'Move job', description: `Move job to "${statusLabel(newStatus)}"?` });
    if (!ok) return;
    const { data: session } = await supabase.auth.getSession();
    ...
    load();
    if (prev) {
      toast({ title: `Moved to ${statusLabel(newStatus)}`, actionLabel: 'Undo', onAction: async () => {
        await fetch('/api/jobs/updateStatus', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) },
      ...
      {JOB_STATUSES.map((col: JobStatus) => (
        ...
        >
          <div className="sticky top-0 z-10 flex items-center justify-between border-b bg-gray-50/80 px-3 py-2 text-sm font-medium backdrop-blur dark:border-gray-800 dark:bg-gray-800/50">
            ...
            <span className="text-xs text-gray-500 dark:text-gray-400">{jobs.filter((j) => j.status === col).length}</span>
          </div>
          <div className="space-y-2 p-2 min-h-[260px]">
            ...
            {jobs
              ...
              .map((j) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-600 dark:text-gray-400">{j.system_type} • {j.capacity_kw ?? '—'} kW</div>
                  <div className="mt-2 flex items-center justify-between gap-2">
                    <a className="inline-block text-xs text-blue-600 dark:text-blue-400" href={`/jobs/${j.id}`}>
                      Open
                    </a>
                    ...
                    <select
                      ...
                      onChange={async (e) => {
                        ...
                        const ok = await confirm({ title: 'Move job', description: `Move job to "${statusLabel(next)}"?` });
                        if (!ok) return;
                        onDrop(j.id, next);
                      }}
                    >
            ...
            {!loading && jobs.filter((j) => j.status === col).length === 0 && (
              <div className="rounded border border-dashed p-3 text-center text-xs text-gray-500 dark:border-gray-800 dark:text-gray-400">No cards</div>
            )}
          </div>
        </div>
      ))}
