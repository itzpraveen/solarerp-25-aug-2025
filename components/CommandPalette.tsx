...
export default function CommandPalette() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (term.length < 2) {
        setResults([
          { type: 'nav', id: '/overview', title: 'Overview', subtitle: 'Dashboard' },
          { type: 'nav', id: '/jobs', title: 'Jobs', subtitle: 'Pipeline' },
          { type: 'nav', id: '/leads', title: 'Leads', subtitle: 'Follow-ups' },
          { type: 'nav', id: '/proposals/new', title: 'New Proposal', subtitle: 'Generate PDF' },
          { type: 'nav', id: '/customers', title: 'Customers', subtitle: 'Directory' },
        ]);
        return;
      }
      setLoading(true);
      try {
        ...
        let jobsById = supabase
          ...
          .ilike('id', `%${term}%`)
          .limit(5) as any;
        if (branchId && branchId !== 'all') jobsById = jobsById.eq('branch_id', branchId);
        ...
        const customerResults: Result[] = ((customers as any[]) || []).map((c: any) => ({
          type: 'customer',
          id: c.id,
          title: c.name || '—',
        ...
        const showActions = term.length < 2 || t.startsWith('new') || t.startsWith('add') || t === '+';
        if (showActions) {
          actions.push(
            { type: 'action', id: 'new:lead', title: 'New Lead', subtitle: 'Create a lead' },
            { type: 'action', id: 'new:customer', title: 'New Customer', subtitle: 'Add a customer' },
            { type: 'action', id: 'new:job', title: 'New Job', subtitle: 'Open Jobs to create' },
          );
        }
        setResults([...actions, ...jobResults, ...customerResults].slice(0, 15));
      } finally {
  ...
  const onOpenRoute = useCallback((r: Result) => {
    if (r.type === 'job') window.location.href = `/jobs/${r.id}`;
    else if (r.type === 'customer') window.location.href = `/customers/${r.id}`;
        ...
        <div className="flex items-center border-b px-3 py-2 dark:border-gray-800">
          <input
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === 'Enter') { const r = results[sel]; if (r) onOpenRoute(r); }
            }}
          />
        </div>
        <div className="max-h-80 overflow-auto py-1">
          {loading && <div className="px-3 py-2 text-sm text-gray-600 dark:text-gray-400">Searching…</div>}
          {nothing && <div className="px-3 py-2 text-sm text-gray-600 dark:text-gray-400">No results</div>}
          {results.map((r, i) => (
            <button
              key={`${r.type}:${r.id}`}
            ...
            >
              <div>
                ...
                {r.subtitle && <div className="text-xs text-gray-600 dark:text-gray-400">{r.subtitle}</div>}
              </div>
              <div className="text-xs rounded border px-1.5 py-0.5 text-gray-600 dark:border-gray-800 dark:text-gray-400">{r.type}</div>
            </button>
          ))}
        </div>
      </div>
