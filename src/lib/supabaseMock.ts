...
function seedDb(): MockDb {
  ...
  const customers = [{ id: 'c1', tenant_id: tenantId, name: 'Alice', phone: '', email: '', address: '' }];
  const jobs = [
    { id: 'j1', tenant_id: tenantId, customer_id: 'c1', status: 'Lead', capacity_kw: '3', system_type: 'On-grid', location: 'Kochi' },
  ];
  const items = [
    { item_code: 'PV-3KW', tenant_id: tenantId, name: '3kW PV Module Set', category: 'PV', unit: 'set', gst_rate: 12, mrp: 150000, preferred_vendor: 'SolarVendor' },
    { item_code: 'INV-3KW', tenant_id: tenantId, name: '3kW Inverter', category: 'Inverter', unit: 'nos', gst_rate: 18, mrp: 45000, preferred_vendor: 'VoltCo' },
  ];
  const kits = [
    { kit_name: 'On-grid 3kW', tenant_id: tenantId, capacity_kw: 3, selling_price: 185000, description: 'Standard on-grid 3kW system' },
  ];
  const kit_items = [
  ...
  return {
    ...
    settings: [{ id: 's1', tenant_id: tenantId, currency: 'INR', default_tax_rate: 18, deposit_percent: 30 }],
    items,
    kits,
    kit_items,
    customers,
...
class MockQuery {
  ...
  async _exec() {
    ...
    if (this.table === 'jobs') {
      if (this._select?.includes('customers(')) {
        rows = rows.map(r => {
          ...
          return { ...r, customers: [{ name: c.name || '—', phone: c.phone || '', email: c.email || '' }] };
        });
      }
    ...
    if (this.table === 'proposals' && this._select?.includes('jobs(')) {
      rows = rows.map(r => {
        ...
        const c = db.customers.find(cu => cu.id === (j as any).customer_id) || {};
    ...
    if (this.table === 'kit_items' && this._select?.includes('items(')) {
      rows = rows.map(r => {
        ...
        return { ...r, items: { name: it.name || '', unit: it.unit || '', preferred_vendor: it.preferred_vendor || '', mrp: it.mrp || 0 } };
      });
    }
  ...
  insert(payload: Row | Row[]) {
    ...
    const withIds = items.map((it) => {
      if (it.id == null) {
        ...
        const prefix = String(this.table).split('_').map(s => s[0]).join('') || 'id';
        return { id: `${prefix}_${Math.random().toString(36).slice(2, 8)}`, ...it };
      }
      return it;
    });
    (db[this.table] as Row[]).push(...withIds);
    return { select: (_?: string) => ({ single: async () => ({ data: withIds[0], error: null }) }) } as any;
  }
  ...
  async delete() {
    const self = this;
    return {
      eq(k: string, v: any) { self.eq(k, v); return this; },
      then(resolve: (v: any) => void, reject?: (e: any) => void) {
        try {
          const db = getDb();
          const all = (db[self.table] as Row[]) || [];
          const keep = all.filter(r => !self.applyFilters([r]).length);
          (db[self.table] as any).length = 0; (db[self.table] as any).push(...keep);
