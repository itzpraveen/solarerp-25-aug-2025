...
export default function CustomerDetail() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setCustomer(c || null);
      const { data: j } = await supabase.from('jobs').select('*').eq('customer_id', params.id);
      setJobs(j || []);
    })();
  }, [params.id]);
    ...
    <div className="space-y-4">
      ...
      <div className="rounded border bg-white p-4">
        ...
        <ul className="mt-2 space-y-2 text-sm">
          {jobs.map((j) => (
            <li key={j.id} className="flex items-center justify-between"><span>{j.system_type} • {j.status}</span><a className="text-blue-600" href={`/jobs/${j.id}`}>Open</a></li>
          ))}
        </ul>
      </div>
    </div>
