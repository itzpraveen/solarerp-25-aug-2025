  ...
  return (
    <div className="space-y-4">
        ...
        <Card>
          <div className="grid grid-cols-1 gap-2 md:grid-cols-4">
            ...
            <Input
              ...
              onChange={(e) =>
                setForm({ ...form, capacity_kw: Number(e.target.value) })
              }
            />
            <Input
              type="number"
            ...
            />
            <Button onClick={add} className="md:col-span-4">
              Save Kit
            </Button>
          </div>
        </Card>
      ...
      {kits.length === 0 ? (
        <EmptyState
          title="No kits yet"
          description="Create standard kit offerings to speed up quoting."
        />
      ) : (
                  ...
                  <div className="w-full grid grid-cols-1 gap-2 md:grid-cols-4">
                    ...
                    <div className="md:col-span-4 flex gap-2">
                      <Button
                        size="sm"
                        onClick={async () => {
                          if ((Number(editForm.capacity_kw) || 0) <= 0) {
                            setErr('Capacity must be > 0');
                            return;
                          }
                          if ((Number(editForm.selling_price) || 0) < 0) {
                          ...
                          await supabase
                            .from('kits')
                            .update({
                              capacity_kw: Number(editForm.capacity_kw) || 0,
                              selling_price:
                                Number(editForm.selling_price) || 0,
                              description: editForm.description,
                            })
                            .eq('kit_name', k.kit_name);
                          setEditing(null);
                          load();
                        }}
                      >
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        onClick={() =>
                          setOpenItemsForKit(
                            openItemsForKit === k.kit_name ? null : k.kit_name,
                          )
                        }
                      >
